...
30 / 94 testcases passed*/
    ...
    public int findPaths(int m, int n, int maxMove, int startRow, int startColumn) {
        ...
        return dp(startRow, startColumn, maxMove, dp)%MODULO;
    }
    ...
    private int dp(int r, int c, int movesLeft, int[][] dp) {
        ...
        if (movesLeft<=0)
            return 0;
        dp[r][c] = (dp[r][c] + dp(r-1,c,movesLeft-1, dp))%MODULO;
        dp[r][c]%=MODULO;
        dp[r][c] = (dp[r][c] + dp(r+1,c,movesLeft-1, dp))%MODULO;
        dp[r][c]%=MODULO;
        dp[r][c] = (dp[r][c] + dp(r, c-1, movesLeft-1, dp))%MODULO;
        dp[r][c]%=MODULO;
        dp[r][c] = (dp[r][c] + dp(r, c+1, movesLeft-1, dp))%MODULO;
        dp[r][c]%=MODULO;
        return dp[r][c]%MODULO;
    }
